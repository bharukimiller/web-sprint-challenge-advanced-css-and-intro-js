...
*/
function getArtistByIndex(artArr, num) {
  return `the artist at index ${num} is ${artArr[num].name}`
}
...
function get20s(artArr){
  ...
  for(let i = 0; i < artArr.length; i++){
    let birthYear = Number(artArr[i].years.substr(0,4))
    let deathYear = Number(artArr[i].years.substr(7,4))
    if(birthYear >= 1900 && deathYear <= 1999){
...
*/
function removeArtist(artArr, num) {
  artArr.splice(num, 1)
  return artArr.length
}
